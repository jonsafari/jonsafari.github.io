...
\begin{tikzpicture}
  ...
  {
	  %ich & habe & eine & Menge & von & leckeren & gesalzenen & Pistazien & gegessen & . \\
	It & has & created & a & hostile & environment & for & development & . \\
    \mbox{} \\[2em]
	%NULL & gew\"{a}hlte & Parlamente & besitzen & unsere & Freiheiten & nicht & . \\
	Es & hat & ein & entwicklungsfeindlicheres & Umfeld & geschaffen & . \\
	%NULL ({ }) gewählte ({ 1 }) Parlamente ({ 2 }) besitzen ({ 5 }) unsere ({ 6 }) Freiheiten ({ 7 }) nicht ({ 3 4 }) . ({ 8 })
  };
	\draw[-] (eg-1-1.south) to (eg-3-1.north);
	\draw[-] (eg-1-2.south) to (eg-3-2.north);
	\draw[-] (eg-1-3.south) to (eg-3-6.north);
	\draw[-] (eg-1-4.south) to (eg-3-3.north);
	\draw[-] (eg-1-5.south) to (eg-3-4.north);
	\draw[-] (eg-1-6.south) to (eg-3-5.north);
	\draw[-] (eg-1-7.south) to (eg-3-4.north);
	\draw[-] (eg-1-8.south) to (eg-3-4.north);
	\draw[-] (eg-1-9.south) to (eg-3-7.north);
\end{tikzpicture}
\end{document}
